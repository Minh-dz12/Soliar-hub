--[[ This file was getting created by tiktoker : @Minh beol ]]

repeat task.wait(0.25) until game:IsLoaded();
getgenv().Image = "rbxassetid://89060867812752";
...
local Window = Fluent:CreateWindow({
	-- con chó nào muốn skid thì đổi tên
    Title = "Soliar hub : Blox fruit",
    SubTitle = "by Tiktoker:@Minh beol",
    TabWidth = 160,
...
local Tabs = {
    ...
    Haki = Window:AddTab({ Title = "Cường hóa", Icon = "" }),
    Settings = Window:AddTab({ Title = "Cài đặt", Icon = "" }),
    Mele = Window:AddTab({ Title = "Mele", Icon = "" }),
    Quest = Window:AddTab({ Title = "Vật phẩm & Nhiệm vụ", Icon = "" }),
    SeaEvent = Window:AddTab({ Title = "Biển", Icon = "" }),
    Mirage = Window:AddTab({ Title = "Tộc & đảo bí ẩn", Icon = "" }),
    Drago = Window:AddTab({ Title = "Dragon dojo", Icon = "" }),
    Prehistoric = Window:AddTab({ Title = "Đảo núi lửa", Icon = "" }),
    Raids = Window:AddTab({ Title = "Tập kích", Icon = "" }),
...
Sea = World1 or World2 or World3 or plr:Kick("❌️Không có hỗ trợ❌️")
Marines = function()
...
Tabs.Main:AddSection("Cài đặt farm")
...
local Initialize = Tabs.Main:AddToggle("Initialize", {
    Title = "Auto click [M1/Melee/Sword]",
    Description = "[ Ko hỗ trợ Gas M1 ]",
    Default = true
})
...
spawn(function()
    while wait(.2) do
        pcall(function()
            CheckingBone:SetDesc(" Có : " .. GetM("Bones"))
        end)
    end
end)
...
spawn(function()
    while wait(.2) do
        pcall(function()
            ...
            if Killed then
                MobKilled:SetDesc(" Đã giết : " .. (500 - Killed))
            end
        end)
    end
end)
...
local CSection = Tabs.Main:AddSection("Berry")

local Berry = Tabs.Quest:AddToggle("Berry", {
    Title = "Nhặt Berry",
    Description = "",
    Default = false
})
...
local CSection = Tabs.Main:AddSection("Rương")
...
local Zou = Tabs.Quest:AddToggle("Zou", {
    Title = "Qua sea 3",
    Description = "",
    Default = false
})
...
local FactoryRaids = Tabs.Quest:AddToggle("FactoryRaids", {
    Title = "Đánh nhà máy",
    Description = "",
    Default = false
})
...
local CSection = Tabs.Main:AddSection("Nguyên liệu")

Test = Tabs.Main:AddDropdown("Test", {
    Title = "Chọn nguyên liệu (bị lỗi nhưng tui ko fix đc:])",
    Values = MaterialList,
    Multi = false,
    Default = nil
})
...
local Bartilo = Tabs.Quest:AddToggle("Bartilo", {
    Title = "Hoàn thành nhiệm vụ Bartilo",
    Description = "",
    Default = false
})
Bartilo:OnChanged(function(Value)
...
local CitizenQ = Tabs.Quest:AddToggle("CitizenQ", {
    Title = "Hoàn thành nhiệm vụ Citizen",
    Description = "",
    Default = false
})
...
Tabs.Haki:AddSection("Haki màu/ Haki")
local Q = Tabs.Haki:AddToggle("Q", {
    Title = "Teleport Đến Barista Cousin",
    Description = "",
    Default = false
})
Q:OnChanged(function(Value)
...
Tabs.Main:AddSection("bản năng & quan sát")
local Q = Tabs.Combat:AddToggle("Q", {
...
Tabs.Mirage:AddSection("Up tộc V3")
...
Tabs.Quest:AddSection("Dao găm bóng tối & mũ Valkyrie")
local Q = Tabs.Quest:AddToggle("Q", {
    Title = "Lấy dao găm bóng tối",
    Description = "",
    Default = false
})
Q:OnChanged(function(Value)
...
local Q = Tabs.Quest:AddToggle("Q", {
    Title = "Mở khóa haki để spawm rip",
    Description = "",
    Default = false
})
Q:OnChanged(function(Value)
...
local SafeModes = Tabs.Settings:AddToggle("SafeModes", {
    ...
    Description = "Sẽ dịch chuyển lên cao khi máu yếu",
    Default = false
})
...
local DisblesVFX = Tabs.Settings:AddToggle("DisblesVFX", {
    Title = "Xóa hiệu ứng chết & Respawned VFX",
    Description = "",
    Default = false
})
DisblesVFX:OnChanged(function(Value)
